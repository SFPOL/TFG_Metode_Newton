...
uniform int specialShadowErrorMode;
...
bool raySurfNewAlg(FlattenedMultiNodeOctreeInfo node, Ray ray, float t_min, float t_max, float t_entry, float t_exit, out HitData hit, float rad, int iOctree){
    float tol = 0.1;
    float sigma2 = 1;
    float mean = 0;
    vec3 dir = normalize(ray.direction);
    vec3 q = ray.origin;
    float cosalph = 1 - 0.000006;

    vec3 x = pointAtParameter(ray, t_entry);
    ...
    while(true){
        ...
        for (int i = node.pointIndex; i < node.pointIndex + node.pointCount; ++i) {
            Point pi ;
            if (iOctree == 0){
                pi = getMultiPoint1(i);
            }
            ...
            if(dot(piq,dir)>length(piq)*cosalph){
                v = pi.position - x;
                float wnorm = exp(-(pow(length(v)-mean,2)/(2*sigma2)));
                position += wnorm*pi.position;
                color += wnorm*pi.color;
                normal += wnorm*pi.normal;
                label += wnorm*pi.label;
                d += wnorm;
                npoints++;
            }
        }
        ...
        float t = dot(v,n)/dot(dir,n);

        if (abs(dot(n, x-p.position))<tol){
            p.position = x;
            setColorHitNew(hit, p, ray, t);
            hit.hitColor.w == 1.0f;
            return true;
        }
        else {
...
bool evaluateMultiNodeWithNoOrder(FlattenedMultiNode currentNode,Ray ray, float t_min, float t_max,float t_entry, float t_exit, out HitData hit, bool dependsGround, bool dependsError, bool dependsExtra, bool shadowing)
{
    ...
    */
    ...
    int numHits = 0;
    if(dependsGround){
        return raySurfNewAlg( currentNode.octreeInfo[0], ray, t_min, t_max, t_entry, t_exit, hit, radius, 0);
    }
    if(dependsError){
        return raySurfNewAlg( currentNode.octreeInfo[1], ray, t_min, t_max, t_entry, t_exit, hit, radiusError, 1);
    }

    if(dependsExtra){
        return raySurfNewAlg( currentNode.octreeInfo[2], ray, t_min, t_max, t_entry, t_exit, hit, radiusError, 2); 
    }
...
int world_has_hit(Ray r, float t_min, float t_max) {
  HitData temp_hit;
  bool hit_anything = false;
  float closest_so_far = t_max;
  if(searchMultiOctree(r, t_min, closest_so_far, temp_hit,true))
  { 
    if(temp_hit.error == true){
        return 2;
    }
    else{
        return 1;
...
vec3 computeShadow(PointLight light, vec3 p, int maxT, bool soft){ 
	...
	if (!soft)
	{
		 ...
		 Ray r = Ray(p, dir);
        int valor = world_has_hit(r, eps, dmax);
		if(valor == 1)
		{
